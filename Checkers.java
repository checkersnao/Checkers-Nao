...
public class Checkers extends JPanel {
   ...
   public Checkers() {
      ...
      board.setBounds(0,0,SIZE,SIZE); // Note:  size MUST be 164-by-164 !
   ...
   private class Board extends JPanel implements ActionListener, MouseListener {
      ...
      public void actionPerformed(ActionEvent evt) {
         ...
         if (src == newGameButton)
            ...
            doResign();
        	 ...
        	 timer.stop();
        	 doClickSquare(legalMoves[0].fromRow, legalMoves[0].fromCol);
         }
      }
      ...
      void doNewGame() {
         ...
         legalMoves = board.getLegalMoves(CheckersData.RED, false);  // Get RED's legal moves.
         ...
         for (int i = 0; i < legalMoves.length; i++)
            if (legalMoves[i].fromRow == row && legalMoves[i].fromCol == col) {
               ...
               if (currentPlayer == CheckersData.RED)
            	  ...
            	  row = legalMoves[i].toRow;
               	  col = legalMoves[i].toCol;
                  repaint();
               	  break;
               }
               repaint();
      ...
      void doMakeMove(CheckersMove move) {
         ...
         if (currentPlayer == CheckersData.RED) {
            ...
            legalMoves = board.getBestMoves(currentPlayer);
            if (legalMoves == null)
            ...
            legalMoves = board.getLegalMoves(currentPlayer, false);
            if (legalMoves == null)
         ...
         repaint();
         if (currentPlayer == CheckersData.BLACK && gameInProgress) {
        	 timer.start();
         }	 
      }  // end doMakeMove();
      ...
      public void paintComponent(Graphics g) {
         ...
         for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
               ...
               case CheckersData.RED:
                  ...
                  g.fillOval(size/12 + col*size, size/12 + row*size, 5*size/6, 5*size/6);
                  break;
               ...
               case CheckersData.RED_KING:
                  g.setColor(Color.RED);
                  g.fillOval(size/12 + col*size, size/12 + row*size, 5*size/6, 5*size/6);
                  g.drawString("K", size/12 + col*size, size/6 + row*size);
                  break;
         ...
         if (gameInProgress && currentPlayer != CheckersData.BLACK) {
            ...
            for (int i = 0; i < legalMoves.length; i++) {
               g.drawRect(legalMoves[i].fromCol*size, legalMoves[i].fromRow*size, size-1, size-1);
               g.drawRect(1 + legalMoves[i].fromCol*size, 1 + legalMoves[i].fromRow*size, size-3, size-3);
            }
            ...
            if (selectedRow >= 0) {
               ...
               g.drawRect(selectedCol*size, selectedRow*size, size-1, size-1);
               g.drawRect(1 + selectedCol*size, 1 + selectedRow*size, size-3, size-3);
               g.setColor(Color.green);
               for (int i = 0; i < legalMoves.length; i++) {
                  if (legalMoves[i].fromCol == selectedCol && legalMoves[i].fromRow == selectedRow) {
                     g.drawRect(legalMoves[i].toCol*size, legalMoves[i].toRow*size, size-1, size-1);
                     g.drawRect(1 + legalMoves[i].toCol*size, 1 + legalMoves[i].toRow*size, size-3, size-3);
                  }
               }
            }
         }
      ...
      public void mousePressed(MouseEvent evt) {
         if (gameInProgress == false)
            ...
            int col = (evt.getX() / (SIZE/8));
            int row = (evt.getY() / (SIZE/8));
   ...
   private static class CheckersData {
       ...
       */
      CheckersMove[] getLegalMoves(int player, boolean jumps) {
      ...
      private boolean inDanger(int player, int r1, int c1) {
    	  return true;
      }
      /**
      ...
      CheckersMove[] getBestMoves (int player) {
    	  
          CheckersMove[] moves = getLegalMoves(player, false);
          
    	  ArrayList<CheckersMove> newMoves = new ArrayList<CheckersMove>();
    	  for (int i = 0; i < moves.length; i++)
    		  newMoves.add(moves[i]);
    	  
    	  if (player == CheckersData.BLACK || player == CheckersData.BLACK_KING) {
    		  
    		  CheckersMove[] jumps = getLegalMoves(CheckersData.RED, true);
    		  ArrayList<CheckersMove> blocks = new ArrayList<CheckersMove>();
    		  
    		  for (int i = 0; jumps != null && i < jumps.length; i++) {
    			  for (int j = 0; j < newMoves.size(); j++) {
    				  
        			  int r1 = newMoves.get(j).fromRow;
        			  int c1 = newMoves.get(j).fromCol;
        			  int r2 = newMoves.get(j).toRow;
        			  int c2 = newMoves.get(j).toCol;
        			  
    				  if ((r1 == jumps[i].toRow) && (c1 == jumps[i].toCol))
    					  blocks.add(new CheckersMove(r1, c1, r2, c2));
    			  }
    		  }
    			  
    		  for (int i = 0; i < newMoves.size(); i++) {
	    			  
	    		int r1 = newMoves.get(i).fromRow;
	    		int c1 = newMoves.get(i).fromCol;
	    		int r2 = newMoves.get(i).toRow;
	    		int c2 = newMoves.get(i).toCol;
	    			  
	    		System.out.printf("Moves: %d, %d to %d %d\n", r1, c1, r2, c2);
	    		int oldPiece = board[r1][c1];
	    		board[r1][c1] = EMPTY;
	    		board[r2][c2] = oldPiece;
	    			  
	    		CheckersMove[] red = getLegalMoves(CheckersData.RED, true);
	    	    	
	    		if (red != null) {
	    			System.out.println(red.length);
	    			
	    			for (int j = 0; j < red.length; j++) {
	    				if (canJump(CheckersData.RED, red[j].fromRow, red[j].fromCol, r2, c2, red[j].toRow, red[j].toCol)) {
	    					newMoves.remove(i--);
		    				break;
		    			}
		    		}
    		...
    		if (newMoves.size() != 0) {
    			System.out.println("AVOID JUMP");
    			CheckersMove[] moveArray = new CheckersMove[newMoves.size()];
    			for (int i = 0; i < newMoves.size(); i++) {
